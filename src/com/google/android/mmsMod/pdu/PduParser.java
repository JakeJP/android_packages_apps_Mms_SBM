...
package com.google.android.mmsMod.pdu;
...
import com.google.android.mmsMod.pdu.NotifyRespInd;
import com.google.android.mmsMod.pdu.RetrieveConf;
import com.google.android.mmsMod.pdu.DeliveryInd;
import com.google.android.mmsMod.pdu.AcknowledgeInd;
import com.google.android.mmsMod.pdu.ReadRecInd;
import com.google.android.mmsMod.pdu.MultimediaMessagePdu;
    ...
    protected PduHeaders parseHeaders(ByteArrayInputStream pduDataStream){
        ...
        while (keepParsing && (pduDataStream.available() > 0)) {
            ...
            if ((headerField >= TEXT_MIN) && (headerField <= TEXT_MAX)) {
                ...
                /* Softbank Japan specific process
                 *    body message summary will go to Subject only when Subject field is empty.
                 */
                if( Arrays.equals( bVal, PduHeaders.X_MMS_VODAFONE_NOTIF_TEXT_STR ) )
                {
	                if (LOCAL_LOGV) { Log.v(LOG_TAG, "Vofacone Header detected: " + new String(bVal)); }
					EncodedStringValue subjectText = headers.getEncodedStringValue( PduHeaders.SUBJECT );
					byte [] textString = parseWapString(pduDataStream, TYPE_TEXT_STRING);
					EncodedStringValue value;
					if( subjectText == null ){
						value = new EncodedStringValue( 0x6A /*utf-8*/ , textString); 
						headers.setEncodedStringValue(value, PduHeaders.SUBJECT);
		                if (LOCAL_LOGV) { Log.v(LOG_TAG, "Vofacone body summary overwrites Subject field: " + new String(textString)); }
					}
					//else { value = subjectText; value.appendTextString( textString ); }
				} else
                if (LOCAL_LOGV) {
